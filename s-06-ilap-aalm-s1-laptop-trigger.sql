...
-- @Fecha   11/dic/2023
-- @Descripcion: Trigger laptop Nodo 3
...
begin
  case
    when inserting then
      --insersion local
      if substr(:new.num_serie,1,1) IN (4,5) then
    ...
    when deleting then
      if substr(:old.num_serie,1,1) IN (4,5) then
        ...
        delete from laptop_f5
          where laptop_id=:old.laptop_id;
      elsif substr(:old.num_serie,1,1) IN (0,1) then
